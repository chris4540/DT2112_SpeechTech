...
\begin{problem}{5.2}
    ...
    the application is able to acheive should be collected.

    In speech systhesis, the completeness of expected functionalities acheived
    ...
    To an end user, the evaluation targets are functionality, user experience.
    1. how can it complete the task 
    2. does the application give a good user experience.
    % Speech Recognition:

    % research instiution; % Test hypotheses
    ...
    % a research and development company; % company selling the underlying technology to service providers.
    ...
    % % For a research instiution, the purpose of evaluation is to test hypothesis.
    % % For example, a group of researchers has a part of modification on the feature
    % extraction part of the speech recognition system. Therefore they would compare
    % the current method and the improved method with objective evaluation like
    % word accuracy for different feature extraction methods. If the new method has
    % a higher accuracy, then it is a strong evidence to show that the new method is better.
