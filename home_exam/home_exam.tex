...
%     1. THE AURORA EXPERIMENTAL FRAMEWORK (asr2000_final_footer.pdf)
% 2. Implementation and Evaluation of an HMM-based Thai Speech Synthesis 
% 3. ITSPOKE: An Intelligent Tutoring Spoken Dialogue System

% the main motivation behind this evaluation:
% 1: evaluate the performance of speech recognition algorithms in noisy conditions
% 2: evaluate the new purposed HMM synthesis system for Thai language.
% 3: Evaluate a text-based tutorial dialogue system.

% Describe the method:
% 1: Prepare the source speech from TIdigits and then add noises to the corpus.
%    Measure the recognition performance with word accuracy at different noise levels.
% 2.1: Evaluate the tone correctness by comparing the F0 contour shapes of synthesized speechs and natural speech
%    across 100 synthesized speech utterances.
% 2.2: Evaluate the Speech Quality between the HMM-based system and the Vaja TTS system, a unit-selection-based TTS system.
%     There are two types of subjective tests: Mean Opinion Score (MOS) and Comparison Category Rating (CCR) are used.
%     In MOS test, listeners are required to evaluate ynthesized speechs in the overall aspect.
%     In CCR test, pairs of speech samples from Vaja and HMMbased systems are scored  in naturalness, clearness, and
% overall aspects
% 3: 1. make students to read a samll document of background material
%    2. pretest their physics knowledge
%    3. use the dialogue system to work through 5 physics problems
%    4. posttest physics knowledge.

% Evaluation involve human judges, automatic measures, or both
% 1: No human judge, automatic measures
% 2.1: No human judge, automatic measures
% 2.2: human judge
% 3: human judges

% suggest something that could improve the evaluation
% 1. collect and use another group of data in more general speech, not digits. But expensive
% 2. more aspects
% 3. The evaluation is too emprical, only task successfulness are counted.
%    Dialogue cost and user satisfaction should be included. # ref PARADISE: A Framework for Evaluating Spoken Dialogue Agents
\end{problem}
\pagebreak
